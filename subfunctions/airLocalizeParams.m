classdef airLocalizeParams < handle & matlab.mixin.Copyable
    ...
    methods
        ...
        function saveLocAndPar(obj,loc,locVars,alData)
            ...
            if strcmp(e,'.par4')
                % replace imgFileName (which can be the input folder name 
                % if batch with current data file name
                p2 = obj.duplicate();
                p2.imgFileName = alData.curImgFile;
        ...
        function [locFileName,parFileName,imgFileName] = ...
            ...
            [d,f,~] = fileparts(alData.curImgFile);
        ...
        function fList = getFList(obj)
            if ismember(obj.fileProcessingMode,{'batch','movieInDir','batchMovie'}) 
                if ~exist(obj.imgFileName,'dir')
                    fList = [];
                    disp(['Could not find data folder ',obj.imgFileName]);
                    return
                else
                    caseSensitive = 0;
                    fList = get_clean_file_list(obj.imgFileName,...
                        obj.imgInclusionString , obj.imgExclusionString,...
                        obj.imgRecursive,caseSensitive);
                end
            else
                if ~exist(obj.imgFileName,'file')
                    fList = [];
                    disp(['Could not find data file ',obj.imgFileName]);
                    return
                else
                    fList = {obj.imgFileName};
                end
            end
        end
...
%             end
            switch direction
                case 'toLegacy'
                    ...
                    p2.data_stackname = alData.curImgFile;
                    varargout{1} = p2;
                case 'fromLegacy'
                    % reset object
                    obj.reset();
